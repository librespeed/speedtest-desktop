...
import com.dosse.speedtest.core.serverSelector.ServerSelector
import com.dosse.speedtest.core.serverSelector.TestPoint
import com.dosse.speedtest.core.worker.SpeedtestWorker
import com.dosse.speedtest.widget.json.JSONArray
import com.dosse.speedtest.widget.json.JSONException
...
class Speedtest {
    ...
    fun addTestPoint(json: com.dosse.speedtest.widget.json.JSONObject?) {
        synchronized(mutex) { addTestPoint(TestPoint(json!!)) }
    }

    fun addTestPoints(json: com.dosse.speedtest.widget.json.JSONArray) {
        synchronized(mutex) {
            for (i in 0 until json.length()) try {
                addTestPoint(json.getJSONObject(i))
            } catch (t: com.dosse.speedtest.widget.json.JSONException) {
            }
        }
    }
    ...
    private object ServerListLoader {
        ...
        fun loadServerList(url: String): Array<TestPoint>? {
            return try {
                ...
                val a = com.dosse.speedtest.widget.json.JSONArray(s)
    ...
    fun start(callback: SpeedtestHandler) {
        synchronized(mutex) {
            ...
            state = 4
            try {
                val extra = com.dosse.speedtest.widget.json.JSONObject()
